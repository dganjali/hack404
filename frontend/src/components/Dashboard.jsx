...
const Dashboard = () => {
  ...
  const filteredShelters = shelters
    ...
    .filter(shelter => {
      if (filterBy === 'high') return shelter.predicted_influx > 50;
      if (filterBy === 'medium') return shelter.predicted_influx >= 20 && shelter.predicted_influx <= 50;
      if (filterBy === 'low') return shelter.predicted_influx < 20;
      return true;
    })
    .sort((a, b) => {
  ...
  const getStatusText = (influx) => {
    if (influx > 50) return 'High Alert';
    if (influx >= 20) return 'Moderate';
    return 'Normal';
  };
    ...
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <div className="dashboard-stats">
          ...
          <div className="stat-item">
            <span className="stat-number">{shelters.filter(s => s.predicted_influx > 50).length}</span>
      ...
      <div className="dashboard-controls">
        ...
        <div className="filters">
          <select value={filterBy} onChange={(e) => setFilterBy(e.target.value)}>
            ...
            <option value="high">High Alert (&gt;50)</option>
            <option value="medium">Moderate (20-50)</option>
            <option value="low">Normal (&lt;20)</option>
          </select>
            ...
            <div className="shelter-details">
              ...
              <div className="detail-item">
                ...
                <span className={`capacity-status ${getStatusColor(shelter.predicted_influx)}`}>
                  {shelter.predicted_influx > 50 ? 'Critical' : 
                   shelter.predicted_influx >= 20 ? 'Moderate' : 'Good'}
                </span>
              </div>
            </div>
